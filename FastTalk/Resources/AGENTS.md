...
## Scope
- This file governs `FastTalk/Resources/`.
...
- Keep the app icon sourced from `FastTalk/Resources/Assets.xcassets/FastTalkAppIcon.appiconset` and keep `ASSETCATALOG_COMPILER_APPICON_NAME`, `CFBundleIconFile`, and `CFBundleIconName` aligned with that name.
...
- Add full macOS icon renditions to the app icon set when replacing the app icon. Prefer the standard `.appiconset` path over reintroducing the removed `.icon` wrapper.
...
- Never change the app icon asset name without updating `project.yml`, the generated project, and `Info.plist` in the same change.
- Never assume a file is bundled just because code has a fallback lookup. Check whether the resource actually exists under `FastTalk/Resources/`.
